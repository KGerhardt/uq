...
if not os.path.isfile(args.input):  
	error('ERROR: Sorry, the input path you have specified is not a file!')
...
if not args.decode:
	...
	class init_status:
		def __init__(self,inFile):
			...
			if self.total % 4 == 0: 
				...
				error('ERROR: The FASTQ file provided contains' + str(self.total) +'rows, which is not divisible by 4!')
			self.current = 0
			self.time = time.time()
	...
	def encoder_fixed(total_reads,bases,qualities,N_qual,dna_bytes_per_row,quality_bytes_per_row,status,file_path,bits_per_base,bits_per_quality):
		with open(file_path,'r') as f:
			...
			#print(total_reads, dna_bytes_per_row)
			...
			if dna_pointer is None or qual_pointer is None: 
				print("ERROR: Your system does not have the required amount of memory to store all of the DNA/QUALITY scores :(")
				exit()
			...
			bases = ''.join(bases.decode())
			qualities = ''.join(qualities.decode())
			N_base = 0

			for row in range(total_reads):
				try:
					next(f)
	...
	def test_patterns(table,filename):
		if args.compressor is None: 
			return [table.nbytes, '0.1'] # If the user isn't using compression, patterning won't make a difference, but keys might, so return size. Default pattern is 0.1 for no particular reason. 
		if filename.startswith('DNA'):	
			pattern = None if args.pattern is None else args.pattern[0] 
		elif filename.startswith('QUAL'): 
			pattern = None if args.pattern is None else args.pattern[1] 
		else: 
	...
	with open(args.input,'r') as f:
		...
		line1, line2, line3, line4 = next(f)[:-1], next(f)[:-1], next(f)[:-1], next(f)[:-1]
		status.update()
		...
		if not line1.startswith('@'): 
			error('ERROR: This does not look like a FASTA/FASTQ file! (first line does not start with @)')
		...
		if not line3.startswith('+'): 
			error('ERROR: This does not look like a FASTA/FASTQ file! (third line does not start with +)')
		...
		if len(line2) != len(line4): 
			error('ERROR: This does not look like a FASTA/FASTQ file! (SEQ and QUAL lines are not the same length)')
		...
		static_qualities = {}
		for base, qual in zip(line2, line4):
			try:
		...
		while True:
			try:
				qname = next(f)[:-1]
				dna = next(f)[:-1]
				#original line was next(f)[0] != '+'; python 3 translates the binary to the ascii value of '+' which is 43
				if next(f)[0] != "+":
					error('ERROR: For entry' + str(status.current) + ' the third line does not start with +')
				qualities = next(f)[:-1]
				status.update()
			except StopIteration: 
				break
			...
			for base,qual in zip(dna,qualities):
				try:
					static_qualities[base][qual] += 1
				except KeyError:
					static_qualities[base] = collections.defaultdict(int)
	...
	def order_seps(qname, prefix, suffix, separators):
		unordered_seps = ('([' + ''.join(sep for sep in separators)+ ']+)') # concatenates separators
	...
	if order_seps(qname,prefix,suffix,separators) == order_seps(line1,prefix,suffix,separators):
		...
		print("	   me about it now i'll get to it sooner rather than later. Sorry about the inconvenience! - john")
		exit()
	...
	if len(prefix):	 
		print('	- all QNAMEs prefixed with:', prefix)
	if len(suffix):	 
		print('	- all QNAMEs suffixed with:', suffix)
	if len(separators): 
		print('	- QNAME field separators:'  , separators, '(', len(separators), 'symbols )')
	print('')
	print('DNA Analysis:')
	print('	- DNA sequences contained the following characters:', ' '.join(sorted(base for base in dna_bases)),'(' + str(len(dna_bases)) + ' in total)')
	print('	- Their distribution is:')
			...
			if len(result) == 1:
				...
				for quality,count in result.items():
					if count == qual_graph[quality]:
						#phred adjust by 33.
						print('	- The base', base ,'consistently had the same quality value of', quality, 'which corresponds to a phred score of', ord(quality) - 33)
	...
	print('QUAL Analysis:')
	for base,result in static_qualities.items():
		print('  [phred score breakdown for nucleotide"'+ base + '"]')
		for qual in sorted(static_qualities[base]):
			percentage = (static_qualities[base][qual]/float(base_graph[base]))*100
			print('	 ', str(ord(qual)-33).rjust(3), '|'+('#'*int(percentage/2)).ljust(50)+'|', ('%.3f'%percentage).rjust(7)+'%', str(static_qualities[base][qual]).rjust(12))
		print('')
	print('  [Total distribution]')
	print('	- the following values were seen as quality scores:',' '.join(sorted(qual for qual in quals)),'(' + str(len(quals)) + ' in total)')
	print('	- Their distribution is:')
	...
	quals	 = (''.join(qual for qual in quals)).encode()
	dna_bases = (''.join(base for base in dna_bases)).encode()
	...
	if len(separators):
		def qname_reader(inFile,prefix,suffix):
			with open(inFile,'r') as f:
				...
				seps = '(.*)'.join(sep for sep in separators)
				
				regex = re.compile(seps)
				while True:
					try:
				...
				if column['format'] == 'mapping':
					if len(column['map']) > entries_read/10:
						try:
							...
							_ = [int(value) for value in column['map']]
							column['min'] = min(_)
							column['max'] = max(_)
		...
		for entries_read, qname in enumerate(qnames):
			if len(qname) != cols:
				if cols == False:
					...
					for col in range(cols): 
						columns.append({ 'name':'QNAME_'+str(col+1), 'format':'mapping', 'map':set() })
				else:
			...
			for idx,column in enumerate(qname):
				...
				if columns[idx]['format'] == 'mapping': 
					columns[idx]['map'].add(column)
				...
				# For now we'll just store the min and max values:
					...
					try:
						column = int(column)
						if   column < columns[idx]['min']: 
							columns[idx]['min'] = column
						elif column > columns[idx]['max']: 
							columns[idx]['max'] = column
			...
			if entries_read == target:
				...
				target *= 2
		if fallback: 
			error('Encoding QNAMEs as strings has not been implemented yet. Please send John a message, ideally with some sample data that generates this message.')
		else: 
			check_format(columns,entries_read) # Check the last bit of data that hadn't made it through yet.
		...
		print('Pass 2 of 4: Skipped as there are no delimiters common to all QNAMEs')
	print('\n')
	...
	if variable_read_lengths: 
		print("Using variable encoder")
		arrays = encoder_variable(status.total,dna_bases,quals,N_qual,dna_columns_needed,qual_columns_needed,status,args.input,bits_per_base,bits_per_quality)
	else:
		print("Using fixed encoder")
		arrays = encoder_fixed(status.total, dna_bases,quals,N_qual,dna_columns_needed,qual_columns_needed,status,args.input,bits_per_base,bits_per_quality)
	...
	if args.test:
		...
		print('\nAll done!')
		if args.compressor is None: 
			print('			 Size	Sort:	Raw Tables:				 Raw stats:' )
		else:					   
			print('Size (compressed)	Sort:	Raw Tables:				 Raw stats:' )
		...
		if args.sort is not None and args.sort != (None,): 
			print(' --sort',args.sort,)
		if args.raw is not None: 
			print(' --raw',' '.join(map(str,args.raw)),)
		if args.pattern is not None: 
			print(' --pattern',' '.join(map(str,args.pattern)),)
		print('')
	...
	config['bases'] = config['bases'].decode()
	config['qualities'] = config['qualities'].decode()
	
	path = os.path.join(temp_directory,'config.json')
	print('\nWriting final config to', path, "...")	
	...
	try:
		...
		with tarfile.open(os.path.join(temp_directory,'temp.uq'), mode='w') as temp_out:
			for f in os.listdir(temp_directory): 
				temp_out.add(os.path.join(temp_directory,f),arcname=f)
		shutil.move(os.path.join(temp_directory,'temp.uq'), args.output)
		for f in os.listdir(temp_directory): 
			os.remove(os.path.join(temp_directory,f))
...
## Below is all the code for decoding a uQ file back to FASTQ:
	...
	if os.path.isfile(args.input):
		...
		error('ERROR: Sorry, the path you have provided for input is not a file - please check it and try again!')
	
	uq = tarfile.open(args.input)

	## Define a few very basic functions:
	def check_if_in_tar(uq,file_name): 
		return file_name in uq.getnames()
	
	def py3_load(uq, file):
		array_file = io.BytesIO()
		array_file.write(uq.extractfile(file).read())
		array_file.seek(0)
		return numpy.load(array_file)
	
	def load_from_tar(uq, file_name, pattern='0.1'):
		if pattern.startswith('0.'): 
			return			    py3_load(uq, file_name)
		else:						
			return numpy.rot90( py3_load(uq, file_name) , -int(pattern[0]))

	## Check all the required stuff is there:
	if check_if_in_tar(uq,'config.json'): 
		config = json.loads(uq.extractfile('config.json').read())
	else: 
		error('ERROR: No config.json file was found in your input path! I cannot decode data without it!')
	
	if check_if_in_tar(uq,'DNA.raw'): 
		DNA = load_from_tar(uq,'DNA.raw',config['pattern'][0])
		...
		error('ERROR: No DNA data was found in this uQ file?!')
	...
	qual_N = dict( (v,k) for k,v in N_qual.items()) # reverse key/val pairs
...
def convert_qname(numpy_array,prefix,suffix,columnType):
	...
	try:
		gen = zip(split_bits(DNA,total_dna_bits,bits_per_base), split_bits(QUAL,total_qual_bits,bits_per_quality), convert_qname(QNAMES, prefix, suffix, columnType))
		if len(N_qual) != 0:
			for dna,qual,qname in gen:
				...
				print(qname)
				print(''.join(dna))
				# I'm seeing the qname here in a lot of publicly avalible files instead of +. Perhaps could add a flag to config.json?
				print('+')			
				print(''.join(qual))
		else:
